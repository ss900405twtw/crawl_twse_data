...
def requests_get(*args1, **args2):
    ...
    while i >= 0:
        try:
            # return requests.get(*args1, timeout=10, **args2)
            # print(*args1, **args2)
...
def crawl_monthly_report(date, conn):
...
def crawl_finance_statement_by_date(date, conn):
    year = date.year
    if date.month == 3:
        season = 4
        year = year - 1
    ...
    if year >= 2019:
        ...
        df = crawl_monthly_report(datetime.datetime(year, month, 1), conn)
        crawl_finance_statement(year, season, df.index.levels[0])
        print("craw finance_statement done!")
    html2db(date, conn)
    return {}
...
def table_exist(conn, table):
    ...
    # return list(conn.execute(
    #     f"select count(*) from {table}"))[0][0] == 1
...
def add_to_sql(conn, name, df):
    ...
    try:
        ret.index.names = ["stock_id", "date"]

        ret.to_sql(name, conn, if_exists='replace', index=True, index_label=['stock_id', 'date'], dtype=dtype)
    except:
...
def update_table(conn, table_name, crawl_function, dates):
    ...
    print('start crawl from ', dates[0] , 'to', dates[-1])
    ...
    for idx, d in enumerate(dates):
        ...
        print(f'crawling {idx}/{len(dates)}...')
#         progress.set_description('crawl' + table_name + str(d))

        data = crawl_function(d, conn)
